...
If you don't see the new system or NGINX instance in the web interface, or (some) metrics aren't being collected, please review the following:
  ...
  2. The `amplify-agent` process is running and updating its [log file]({{< ref "/amplify/nginx-amplify-agent/install/configuring-amplify-agent#agent-logfile" >}}). You can check if the process is running by executing `ps aux | grep amplify-agent`. If it's not running, try restarting the service with `sudo service amplify-agent restart` and check the log file for any error messages.
  ...
  6. The time is set correctly. If the time on the system where NGINX Amplify Agent runs is ahead or behind the world's clock, you won't be able to see the graphs. Use `date` to check the current system time and `ntpdate` or `chrony` to synchronize it with a time server if necessary.
  ...
  11. The system DNS resolver is correctly configured, and *receiver.amplify.nginx.com* can be successfully resolved. Test this by running `nslookup receiver.amplify.nginx.com` or `dig receiver.amplify.nginx.com`. If these commands fail, check your DNS settings in `/etc/resolv.conf`.
  12. Outbound TLS/SSL from the system to *receiver.amplify.nginx.com* is not restricted. This can be confirmed with `curl -v https://receiver.amplify.nginx.com`. If you encounter SSL errors, ensure that your system's CA certificates are up to date. [Configure a proxy server]({{< ref "/amplify/nginx-amplify-agent/install/configuring-amplify-agent#setting-up-a-proxy" >}}) for NGINX Amplify Agent if required.
